...
function wpseo_local_show_opening_hours( $atts, $show_schema = true, $standalone = true ) {
	...
	foreach ( $locations as $location ) {
		...
		if ( ( wpseo_has_multiple_locations() && 'on' === get_post_meta( $location['post_id'], '_wpseo_open_247', true ) ) || ( ! wpseo_has_multiple_locations() && isset( $options['open_247'] ) && 'on' === $options['open_247'] ) ) {
			...
			$location_datetime   = $timezone_repository->get_location_datetime();
			...
			if ( ! $show_days == 0 ) {
				foreach ( $days as $key => $day ) {
					...
					if ( ! empty( $location_datetime ) && $key === strtolower( $location_datetime->format( 'l' ) ) && $timezone_repository->is_location_open() && $show_open_now_label ) {
	...
	// Add filter to add optional output. This is for example used to add JSON+LD.
	if ( true !== $atts['hide_json_ld'] && false !== $standalone ) {
