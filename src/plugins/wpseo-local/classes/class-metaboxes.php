			...
			<div id="wpseo_add_map" style="display:none;">
				<div class="wrap">
					<div>
						...
						<?php if ( wpseo_has_multiple_locations() && ! empty( $locations ) ) { ?>
							<div style="padding:15px 15px 0 15px;">
								...
								<select id="wpseo_map_location_id" onchange="WPSEO_Map_Change_Location( this )">
									<option value=""> -- <?php esc_html_e( 'Select a location', 'yoast-local-seo' ); ?> --</option>
									<?php
									$repo      = new WPSEO_Local_Locations_Repository();
									$locations = $repo->get( array(), false );

									if ( ! empty( $locations ) ) {
										echo '<option value="all">' . esc_html__( 'All locations', 'yoast-local-seo' ) . '</option>';

										foreach ( $locations as $location_id ) {
											?>
											<option value="<?php echo esc_attr( $location_id ); ?>" <?php selected( $location_id, get_the_ID(), true ); ?>><?php echo esc_html( get_the_title( $location_id ) ); ?></option>
											<?php
										}
									}
									?>
								...
								<select id="wpseo_map_center_location_id" <?php echo( in_array( get_the_ID(), $locations ) ? 'disabled' : '' ); ?>>
									...
									if ( ! empty( $locations ) ) {
										echo '<option value="">' . esc_html__( 'All locations', 'yoast-local-seo' ) . '</option>';

										foreach ( $locations as $location_id ) {
											?>
											<option value="<?php echo esc_attr( $location_id ); ?>"><?php echo esc_html( get_the_title( $location_id ) ); ?></option>
											<?php
										}
									}
									?>
						...
						<?php
						if ( wpseo_has_multiple_locations() && empty( $locations ) ) {
							echo '<p>';
							printf(
					...
					<div>
						...
						<?php if ( wpseo_has_multiple_locations() && ! empty( $locations ) ) { ?>
							<div style="padding:15px 15px 0 15px;">
								...
								<select id="wpseo_address_location_id" onchange="WPSEO_Address_Change_Order( this );">
									...
									if ( ! empty( $locations ) ) {
										echo '<option value="all">' . esc_html__( 'Show all locations', 'yoast-local-seo' ) . '</option>';
						...
						<?php if ( ( wpseo_has_multiple_locations() && ! empty( $locations ) ) || ! wpseo_has_multiple_locations() ) { ?>
							<div style="padding:15px 15px 0 15px;">
								...
								<label for="wpseo_show_price_range"><input type="checkbox" id="wpseo_show_price_range"/> <?php esc_html_e( 'Show Price Range', 'yoast-local-seo' ); ?>
								</label><br>
