...
if ( ! class_exists( 'WPSEO_Local_Core' ) ) {
	...
	class WPSEO_Local_Core {
		...
		/**
		 * Holds an instance of the license manager class.
		 *
		...
		public function do_upgrade() {
			...
			if ( version_compare( $options['version'], '7.2', '<' ) ) {
				self::update_yoast_seo_settings();
			}
					...
					if ( true === $update_option ) {
						...
						$notification        = new Yoast_Notification(
							sprintf(
								...
								__( 'In order to make full use of Yoast SEO: Local functionality, we have changed the setting for \'Person or Company\' under %1$sYoast SEO Search Appearance%2$s to \'Company\'', 'yoast-local-seo' ),
			...
			if ( class_exists( 'Yoast_Notification_Center' ) ) {
				...
				if ( 'company' !== $new_value['company_or_person'] ) {
					$notification = new Yoast_Notification(
						sprintf(
							/* translators: 1: link open tag; 2: link close tag. */
							__( 'In order to make full use of Yoast SEO: Local functionality, you should consider changing the setting for \'Person or Company\' under %1$sYoast SEO Search Appearance%2$s to \'Company\'', 'yoast-local-seo' ),
							'<a href="' . esc_url( admin_url( 'admin.php?page=wpseo_titles' ) ) . '">',
							'</a>'
						),
						array(
