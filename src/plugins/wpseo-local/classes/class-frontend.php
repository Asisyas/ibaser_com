		...
		public function opengraph_location() {
			if ( false === wpseo_has_multiple_locations() || is_singular( 'wpseo_locations' ) || ( 'on' === WPSEO_Meta::get_value( 'opengraph-local' ) && ! wpseo_has_multiple_locations() ) ) {
				...
				if ( ! $hide_opening_hours ) {
					...
					if ( wpseo_has_multiple_locations() ) {
						$open_247 = get_post_meta( get_the_ID(), '_wpseo_open_247', true );
					}
					else {
						$open_247 = isset( $options['open_247'] ) ? esc_attr( $options['open_247'] ) : '';
					}

					// Force all days to show 24h opening times.
					foreach ( $days as $key => $day ) {
						$oh_tmp = $opening_hours_repo->get_opening_hours( $key, ( ( '' !== get_the_ID() ) ? get_the_ID() : 'options' ), $this->options );

						if ( $oh_tmp['value_from'] == 'closed' ) {
							continue;
						}

						$opening_hours[ $key ][] = array(
							'from' => ( ( 'on' === $oh_tmp['open_24h'] || 'on' === $open_247 ) ? '0:00' : $oh_tmp['value_from'] ),
							'to'   => ( ( 'on' === $oh_tmp['open_24h'] || 'on' === $open_247 ) ? '23:59' : $oh_tmp['value_to'] ),
						);

						if ( isset( $oh_tmp['use_multiple_times'] ) && true === $oh_tmp['use_multiple_times'] && $oh_tmp['value_second_from'] !== 'closed' && ! $oh_tmp['open_24h'] && 'on' !== $open_247 ) {
							$opening_hours[ $key ][] = array(
								'from' => $oh_tmp['value_second_from'],
								'to'   => $oh_tmp['value_second_to'],
							);
						}
					}
				}
				...
				foreach ( $locations as $location_id => $location_data ) {
					...
					echo '<meta property="business:contact_data:country_name" content="' . self::get_country( $location_data['business_country'] ) . '"/>' . "\n";
					echo '<meta property="business:contact_data:postal_code" content="' . esc_attr( $location_data['business_zipcode'] ) . '"/>' . "\n";
					echo '<meta property="business:contact_data:website" content="' . esc_url( trailingslashit( WPSEO_Sitemaps_Router::get_base_url( '' ) ) ) . '"/>' . "\n";
					...
					if ( ! empty( $opening_hours ) ) {
						foreach ( $opening_hours as $key => $opening_hours_day ) {
							echo '<meta property="business:hours:day" content="' . esc_attr( $key ) . '"/>' . "\n";

							if ( ! isset( $opening_hours_day['247'] ) ) {
								foreach ( $opening_hours_day as $times ) {
									echo '<meta property="business:hours:start" content="' . esc_attr( $times['from'] ) . '"/>' . "\n";
									echo '<meta property="business:hours:end" content="' . esc_attr( $times['to'] ) . '"/>' . "\n";
								}
							}
						}
					}
